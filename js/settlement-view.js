...
// Set to asset URL when artwork is ready; null uses SVG art as fallback.
const SV_BG_IMAGE_TOWN      = null; // '/assets/images/settlement-town.jpg'
const SV_BG_IMAGE_OUTSKIRTS = null; // '/assets/images/settlement-outskirts.jpg'

// ── Slot definitions ──────────────────────────────────────────────────────────
// Each area fills the FULL scene width when its tab is active.
const SV_SLOTS = [
  // Town
...
.sv-bg-img.sv-img-loaded { opacity: 1; }
.sv-bg {
...
.sv-bg-svg  { position: absolute; bottom: 0; left: 0; width: 100%; height: 55%; }
.sv-mid-svg { position: absolute; bottom: 0; left: 0; width: 100%; height: 60%; }
...
.sv-build-panel {
  ...
  padding: 20px 20px 32px; z-index: 20;
...
function _ensureSvDOM() {
  ...
  const townSlots = SV_SLOTS.filter(s => s.area === 'town');
  const outSlots  = SV_SLOTS.filter(s => s.area === 'outskirts');

  root.innerHTML = `
    <div class="sv-scene" id="sv-scene">
      <div class="sv-layer sv-bg" id="sv-layer-bg">
        <img class="sv-bg-img" id="sv-bg-img" src="" alt=""
             onload="this.classList.add('sv-img-loaded')"
             onerror="this.style.display='none'">
        ${_bgSvg()}
      </div>
      <div class="sv-layer" id="sv-layer-mid">${_midSvg()}</div>
      <div class="sv-layer sv-fg-layer" id="sv-layer-fg-town">
        ${townSlots.map(sl => `<div class="sv-slot" id="sv-slot-${sl.id}" style="left:${sl.x}%;top:${sl.y}%" data-slot="${sl.id}"></div>`).join('')}
      </div>
      <div class="sv-layer sv-fg-layer sv-area-hidden" id="sv-layer-fg-outskirts">
        ${outSlots.map(sl => `<div class="sv-slot" id="sv-slot-${sl.id}" style="left:${sl.x}%;top:${sl.y}%" data-slot="${sl.id}"></div>`).join('')}
      </div>
    </div>
...
function _midSvg() {
  return '<svg class="sv-mid-svg" viewBox="0 0 1000 480" preserveAspectRatio="none" xmlns="http://www.w3.org/2000/svg">'
    ...
    + '<path d="M180,480 Q265,428 328,408 Q415,388 500,396 Q585,404 644,416 Q708,430 770,480" stroke="rgba(195,170,110,0.38)" stroke-width="20" fill="none" stroke-linecap="round"/>'
    + '<path d="M830,480 Q848,440 862,410 Q878,378 908,362 Q938,348 968,338 Q990,328 1060,318" stroke="rgba(90,155,215,0.32)" stroke-width="16" fill="none" stroke-linecap="round"/>'
    + '</svg>';
}
...
async function openSettlementView() {
  ...
  const commBar = document.querySelector('.community-bar');
  const topOffset = (topbar  ? topbar.offsetHeight  : 0)
                  + (commBar ? commBar.offsetHeight : 0);
...
function _svSwitchArea(area, force) {
  ...
  _svActiveArea = area;
  _closeBuildPanel();
  const tl = document.getElementById('sv-layer-fg-town');
  const ol = document.getElementById('sv-layer-fg-outskirts');
  if (area === 'town') {
...
// ── Background image ──────────────────────────────────────────────────────────
function _svUpdateBgImage() {
  var img = document.getElementById('sv-bg-img');
  if (!img) return;
  var url = _svActiveArea === 'town' ? SV_BG_IMAGE_TOWN : SV_BG_IMAGE_OUTSKIRTS;
  if (url) {
    img.classList.remove('sv-img-loaded');
...
function _renderScene() {
  for (const slot of SV_SLOTS) {
    ...
    if (occupant) {
      ...
      el.onclick = function() { _openBuildPanel(slot, occupant); };
    } else {
      const anyAvail = slot.accepts.some(function(id) {
        const b = _svBuildings.find(function(b) { return b.id === id; });
        return b && b.currentLevel === 0 && b.requiresMet;
      });
      const allLocked = slot.accepts.every(function(id) {
        const b = _svBuildings.find(function(b) { return b.id === id; });
        return !b || !b.requiresMet;
      });
      el.appendChild(_buildEmptyEl(slot, anyAvail, allLocked));
      if (anyAvail) el.onclick = function() { _openBuildPanel(slot, null); };
    }
  }
}
...
function _occupantFor(slot) {
  for (const id of slot.accepts) {
    const b = _svBuildings.find(function(b) { return b.id === id && b.currentLevel > 0; });
    if (b) return b;
  }
  return null;
}
...
function _buildEmptyEl(slot, available, locked) {
  ...
  div.innerHTML = '<div class="sv-slot-ring"><span class="sv-slot-plus">' + (locked ? '🔒' : '+') + '</span></div><div class="sv-slot-hint">' + slot.label + '</div>';
  return div;
}

function _buildOccupiedEl(slot, building) {
  const vis  = SV_VISUALS[building.id] || { emoji:'🏛', bodyColor:'#555', roofColor:'#333' };
  const szMap = { lg:{w:64,h:52,rw:72}, md:{w:52,h:44,rw:58}, sm:{w:40,h:36,rw:46} };
  const sz   = szMap[slot.size] || szMap.md;
...
function _openBuildPanel(slot, occupant) {
  ...
  let html = '<div class="sv-bp-header"><span class="sv-bp-loc">' + slot.label + '</span><span class="sv-bp-tag ' + slot.area + '">' + areaTag + '</span><button class="sv-bp-close" onclick="_closeBuildPanel()">\xD7</button></div>';

  if (occupant) {
    const vis   = SV_VISUALS[occupant.id] || { emoji:'🏛', flavor:'' };
    const isMax = occupant.currentLevel >= occupant.maxLevel;
    const costStr = occupant.cost ? Object.entries(occupant.cost).map(function(e) { return _resIcon(e[0]) + ' ' + e[1]; }).join('  ') : '';
    html += '<div class="sv-occ-head"><span class="sv-occ-emoji">' + vis.emoji + '</span><div><div class="sv-occ-name">' + occupant.label + '</div><div class="sv-occ-lv">Level ' + occupant.currentLevel + ' / ' + occupant.maxLevel + '</div></div>'
      + (isMax ? '<span class="sv-bp-lv-badge" style="margin-left:auto">★ MAX</span>' : '<button class="sv-bp-upgrade-btn" style="margin-left:auto" onclick="_svBuild(\'' + occupant.id + '\')">↑ Upgrade' + (costStr ? ' — ' + costStr : '') + '</button>')
      + '</div><div class="sv-occ-desc">' + (occupant.desc || '') + '</div><div class="sv-occ-flavor">' + (vis.flavor || '') + '</div>';
  } else {
    const opts = slot.accepts.map(function(id) {
      const b   = _svBuildings.find(function(b) { return b.id === id; });
      if (!b) return '';
      const vis = SV_VISUALS[id] || { emoji:'🏛' };
      const costStr = b.cost ? Object.entries(b.cost).map(function(e) { return _resIcon(e[0]) + ' ' + e[1]; }).join('  ') : 'Free';
      if (b.requiresMet && b.currentLevel === 0) {
        return '<div class="sv-bp-opt available"><span class="sv-bp-emoji">' + vis.emoji + '</span><div class="sv-bp-info"><div class="sv-bp-name">' + b.label + '</div><div class="sv-bp-desc">' + (b.desc || '') + '</div><div class="sv-bp-cost">' + costStr + '</div></div><div class="sv-bp-action"><button class="sv-bp-build-btn" onclick="_svBuild(\'' + id + '\')">Build</button></div></div>';
      }
      if (!b.requiresMet) {
        return '<div class="sv-bp-opt locked"><span class="sv-bp-emoji">' + vis.emoji + '</span><div class="sv-bp-info"><div class="sv-bp-name">' + b.label + '</div><div class="sv-bp-desc">Requires other buildings first.</div></div><span style="font-size:18px">🔒</span></div>';
      }
      return '';
    }).join('');
...
function _resIcon(res) {
  var icons = { food:'🌿', timber:'🌲', stone:'🪨', metal:'⚙️', wealth:'🪙' };
...
function _tickParallax() {
  ...
  const tFg = document.getElementById('sv-layer-fg-town');
  const oFg = document.getElementById('sv-layer-fg-outskirts');
  ...
  if (tFg) tFg.style.transform = 'translate(' + (_svCurX*16) + 'px,' + (_svCurY*9) + 'px)';
  if (oFg) oFg.style.transform = 'translate(' + (_svCurX*16) + 'px,' + (_svCurY*9) + 'px)';
