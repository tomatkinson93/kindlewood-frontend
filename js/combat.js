...
router.post('/resolve', requireAuth, async (req, res) => {
  try {
    ...
    if (questRun) {
      ...
      // If the resumed quest is already past completion (the pause was
      // longer than the time remaining), or if it lands very close to
      // completion soon, we want the player to see the "returned" toast
      // immediately — not 15-60s later when the safety-net poller runs.
      // Resolve now: if the quest has elapsed it'll flip to completed and
      // publish quest_resolved synchronously. If not, this is a cheap no-op
      // (the SELECT returns no rows).
      //
      // We don't await this in the response path — fire-and-forget so the
      // client gets its 200 back quickly. The publish will arrive over the
      // SSE stream a moment later if there's anything to resolve.
